					...
					unsigned long count, void *data)
{
	...
	if (b == '0') {
		...
		if (!has_lpm_enabled) {
			...
			bsi->uport = msm_hs_get_uart_port(BT_PORT_NUM);
...
static int __init bluesleep_init(void)
{
	...
	ent = create_proc_entry("btwake", S_IRUGO | S_IWUSR | S_IWGRP, sleep_dir);
	if (ent == NULL) {
	...
	ent = create_proc_entry("proto", S_IRUGO | S_IWUSR | S_IWGRP, sleep_dir);
	if (ent == NULL) {
	...
	/* read/write proc entries */
	ent = create_proc_entry("lpm", S_IRUGO | S_IWUSR | S_IWGRP, sleep_dir);
	if (ent == NULL) {
	...
	/* read/write proc entries */
	ent = create_proc_entry("btwrite", S_IRUGO | S_IWUSR | S_IWGRP, sleep_dir);
	if (ent == NULL) {
